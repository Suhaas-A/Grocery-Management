...
def home():
    ...
    items = Items.query.all()
    bill_items = Bill_items.query.all()

    bl = []
    il =[]
    bil = []
    for a in bills:
        bl.append([a.id, a.name, a.created_at, a.phone])
    for b in bill_items:
        bil.append([b.id, b.bill_id, b.item_id, b.quantity])
    for c in items:
        il.append([c.id, c.item, c.price])

    return {'bills': bl, 'items': il, 'bill_items': bil}
